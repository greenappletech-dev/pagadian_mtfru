    ...
    <div class="main-container p-4">
        ...
        <div class="user-button d-flex justify-content-between pt-3 pb-3">
            <ul class="breadcrumb-1 pl-0 m-0">
                <li class="d-inline align-middle" style="font-size: 12px">FVR</li>
                    ...
                    <div class="col-md-12">
                        <v-client-table
                            ...
                            <span slot="action" slot-scope="{row}">
                                <button class="btn btn-warning d-inline-block" v-on:click="storeForRenewal(row.id)"><i class="fas fa-check mr-1"></i>Renew</button>
                            </span>
                        </v-client-table>
                    </div>
                </div>
...
export default {
    ...
    data() {
        return {
            columns: [ 'transact_date', 'operator','body_number', 'boat_desc','make_type', 'engine_motor_no', 'approve_date', 'validity_date', 'or_number', 'amount', 'action'],
            tableData: [],
            options: {
                headings: {
                ...
                sortable: ['transact_date', 'body_number'],
                filterable: ['transact_date', 'body_number'],
                templates: {
                    ...
                    validity_date: function(h, row) {
                        return row.transact_date !== null ? moment(row.validity_date).format('MM-DD-YYYY') : null;
                    },
                },
                texts : {
                    filter: 'Search:',
    ...
    methods: {
        ...
        initialData() {
            axios.get('fvr_renewal/getdata').then(response => {
        ...
        getDataFiltered() {
            ...
            axios.get('fvr_renewal/filter/' + this.fromDateValue + '/' + this.toDateValue + optional_route)
                .then(response => {
        ...
        storeForRenewal(id) {
            location.href = 'fvr_entry/renew/' + id;
        },

        exportExcel() {
            let optional_route = this.barangayCodeValue === '' ? '/null' : '/'  + this.barangayCodeValue;
            window.open('fvr_renewal/export/' + this.fromDateValue + '/' + this.toDateValue + optional_route);
        },
