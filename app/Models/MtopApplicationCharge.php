...
class MtopApplicationCharge extends Model
{
    ...
    public function fetchDataById($id) {
        return MTOPApplicationCharge::leftJoin('otherinc', 'otherinc.id' , 'mtop_application_charges.otherinc_id')->select('mtop_application_charges.*', 'otherinc.inc_desc as name')->where('mtop_application_id', $id)->orderBy('mtop_application_charges.id')->get();
    }
    ...
    public function fetchDataForReport($barangay_id, $from, $to) {
        return MTOPApplicationCharge::leftJoin('mtop_applications', 'mtop_applications.id', 'mtop_application_charges.mtop_application_id')
            ...
            ->select('mtop_applications.*', 'mtop_applications.id as application_id', 'mtop_application_charges.*', 'taxpayer.full_name' ,'barangay.brgy_code as brgy_code', 'barangay.brgy_desc as brgy_desc')
